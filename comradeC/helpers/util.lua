...
local w = d * 7
local y = d * 365.25
util.years = function(time)
  return time * y
end
util.weeks = function(time)
  return time * w
end
util.days = function(time)
  return time * d
end
util.hours = function(time)
  return time * h
end
util.minutes = function(time)
  return time * m
end
util.seconds = function(time)
  return time * s
end
util.formatLong = function(ms)
  local msAbs = math.abs(ms)
  if msAbs >= d then
...
util.bulkDelete = function(msg, messages)
  ...
  elseif type(messages) == 'number' then
    return msg.channel:bulkDelete(msg.channel:getMessages(messages))
  end
end
